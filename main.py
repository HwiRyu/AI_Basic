...
class Neural_Net:
    ...
    def back_propagation(self, input, target):
        ...
        for i in range(1, N + 1):
            self.dL[i] = mse_derivative(Y, T).T @ softmax_derivative(self.L[N])

            for j in range(N, i, -1):
                self.dL[i] = self.dL[i] @ self.W[j] @ np.diagflat(ReLU_Derivative(self.L[j - 1]))
